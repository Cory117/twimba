...
document.addEventListener('click', function(e){
    if(e.target.dataset.like){
       handleLikeClick(e.target.dataset.like) 
        ...
        handleReplyBtnClick(e.target.dataset.response)
    }
})
...
function handleReplyBtnClick(replyId){
    ...
    const replyText = tweetsData.filter(function(reply){
        return reply.uuid === replyId
    })[0]
    
    if(replyInput.value){
        replyText.replies.push({
            handle: `@Scrimba`,
            profilePic: `images/scrimbalogo.png`,
...
<div class="tweet">
    ...
    <div class="hidden" id="replies-${tweet.uuid}">
        ...
        <button id="reply-btn" data-response="${tweet.uuid}">Reply</button>
    </div>
</div>
`
   })
